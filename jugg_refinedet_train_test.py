...
from data import VOCroot, COCOroot,Juggroot, VOC_320, AnnotationTransform, COCODetection, VOCDetection, detection_collate, BaseTransform, preproc,VOC_512, COCOJuggdet
...
if args.dataset == 'VOC':
    testset = VOCDetection(
        VOCroot, ['test_0711'], None, AnnotationTransform())
    train_dataset = VOCDetection(VOCroot, train_sets, preproc(
        img_dim, rgb_means, p), AnnotationTransform())
elif args.dataset == 'COCO':
    testset = COCODetection(
        Juggroot, [('2014', 'minival')], None)
    train_dataset = COCODetection(Juggroot, train_sets, preproc(
        img_dim, rgb_means, p))
else:
